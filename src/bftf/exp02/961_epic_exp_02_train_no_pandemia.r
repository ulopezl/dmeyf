...
# Experimento 02 para comparar la pandemia. 
# Entrenamiento entre 201801 y 201912
...
kscript         <- "961_epic_exp_02_train_meses_pandemia"

karch_dataset    <- "./datasets/datasets_dataset_epic_RECORTADO_v951_PRUEBA_RECORTE.csv.gz" # EN exp 01 al crear con epic dataset no le cambié el nombre y quedó este.
...
ktrain_mes_hasta    <- 201912  #Obviamente, solo puedo entrenar hasta 202011
ktrain_mes_desde    <- 201801
ktrain_meses_malos  <- c( 202006 )  #meses que quiero excluir del entrenamiento


kgen_mes_hasta    <- 201912   #La generacion final para Kaggle, sin undersampling
...
hs <- makeParamSet( 
         ...
         #AGREGO ESTOS HIPERPARAMETROS QUE ANTES ESTABAN FIJOS
         makeNumericParam("lambda_l1",       lower=    0.0   , upper= 200.0),
         makeNumericParam("lambda_l2",       lower=    0.0   , upper= 200.0),
         makeIntegerParam("max_bin",       lower=    2L   , upper= 200L)         
        )
