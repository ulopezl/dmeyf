...
palancas$version  <- "vidapropia_ori_vars_profe_and_mas_ranked_YMAS"   #Muy importante, ir cambiando la version
...
AgregarVariables  <- function( dataset )
{
  ...
  #variables andrea
  dataset[ , AND_total_transacciones_credito := rowSums( cbind( ctarjeta_visa_transacciones,  ctarjeta_master_transacciones) ,na.rm=TRUE ) ]
  dataset[ , AND_monto_deuda_total:= rowSums( cbind( mprestamos_personales,  mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , AND_cantidad_seguros := rowSums( cbind( cseguro_vida,  cseguro_auto, cseguro_vivienda, cseguro_accidentes_personales) , na.rm=TRUE ) ]
  dataset[ , AND_mpayroll_div_cpayrroll:= mpayroll  / cpayroll_trx ]
  dataset[ , AND_cantidad_comisiones := rowSums( cbind( ccomisiones_mantenimiento,  ccomisiones_otras) , na.rm=TRUE ) ]
  dataset[ , AND_monto_comisiones := rowSums( cbind( mcomisiones_otras,  mcomisiones_mantenimiento) , na.rm=TRUE ) ]
  dataset[ , AND_cantidad_debitos_automaticos := rowSums( cbind( ccuenta_debitos_automaticos,  ctarjeta_visa_debitos_automaticos, ctarjeta_master_debitos_automaticos) , na.rm=TRUE ) ]
  dataset[ , AND_monto_debitos_automaticos := rowSums( cbind( mcuenta_debitos_automaticos,  mttarjeta_visa_debitos_automaticos, mttarjeta_master_debitos_automaticos) , na.rm=TRUE ) ]
  dataset[ , AND_cantidad_consumos_tarjetas_cred := rowSums( cbind( Master_cconsumos,  Visa_cconsumos) , na.rm=TRUE ) ]
  dataset[ , AND_monto_consumos_tarjetas_cred := rowSums( cbind( mtarjeta_visa_consumo,  mtarjeta_master_consumo) , na.rm=TRUE ) ]
  dataset[ , AND_limite_compras := rowSums( cbind( Visa_mlimitecompra,  Master_mlimitecompra) , na.rm=TRUE ) ]
  dataset[ , AND_consumo_tarjeta_div_limite_compra := AND_monto_consumos_tarjetas_cred / AND_limite_compras ]
  dataset[ , AND_monto_ganancia_banco := rowSums( cbind( mactivos_margen,  mpasivos_margen) , na.rm=TRUE ) ]
  dataset[ , AND_mcuentas_saldo_sum_mpayroll := rowSums( cbind( mcuentas_saldo,  mpayroll) , na.rm=TRUE ) ]
  dataset[ , AND_monto_consumo_credito_sum_monto_transacciones := rowSums( cbind( AND_monto_consumos_tarjetas_cred,  mautoservicio) , na.rm=TRUE ) ]
  dataset[ , AND_consumos_tarjeta_sum_mpayroll := rowSums( cbind( AND_monto_consumos_tarjetas_cred,  mpayroll) , na.rm=TRUE ) ]
  
  ############# divs y prods ##########
  dataset[ , AND_ctrx_quarter_prod_cantidad_comisiones := AND_cantidad_comisiones  * ctrx_quarter ]
  dataset[ , AND_cantidad_comisiones_div_cpayroll_trx := AND_cantidad_comisiones  / cpayroll_trx ]
  dataset[ , AND_ctarjeta_transacciones_prod_mcaja_ahorro := AND_total_transacciones_credito * mcaja_ahorro ]
  dataset[ , AND_ctrx_quarter_prod_mtarjeta_consumo := ctrx_quarter * AND_monto_consumos_tarjetas_cred ]
  dataset[ , AND_mtarjeta_consumo_prod_mcaja_ahorro := mcaja_ahorro * AND_monto_consumos_tarjetas_cred ]                 
  dataset[ , AND_ctrx_quarter_prod_ctarjeta_transacciones := ctrx_quarter * AND_total_transacciones_credito ]            
  dataset[ , AND_mcaja_ahorro_prod_ccomisiones := mcaja_ahorro * AND_cantidad_comisiones ]
  dataset[ , AND_mtarjeta_consumo_prod_ccomisiones := AND_monto_consumos_tarjetas_cred * AND_cantidad_comisiones ]        
  dataset[ , AND_ctarjeta_transacciones_prod_comisiones := AND_total_transacciones_credito * AND_cantidad_comisiones ]
  dataset[ , AND_ccomisiones_div_ctarjeta_transacciones := AND_cantidad_comisiones / AND_total_transacciones_credito ]  
  
  ###### divs 2da comp #####
  dataset[ , AND_U_mtarjeta_visa_consumo_div_cpayroll_trx := mtarjeta_visa_consumo / cpayroll_trx ]
  dataset[ , AND_U_ctrx_quarter_div_foto_mes := ctrx_quarter / foto_mes ]
  dataset[ , AND_U_mcuentas_saldo_div_mprestamos_personales := mcuentas_saldo / mprestamos_personales ]
  dataset[ , AND_U_foto_mes_div_ctrx_quarter := foto_mes / ctrx_quarter ]
  dataset[ , AND_U_mv_status01_div_mdescubierto_preacordado := mv_status01 / mdescubierto_preacordado ]
  dataset[ , AND_U_mv_status01_div_cpayroll_trx := mv_status01 / cpayroll_trx ]
  dataset[ , AND_U_mcuenta_corriente_div_mtarjeta_visa_consumo := mcuenta_corriente / mtarjeta_visa_consumo ]
  dataset[ , AND_U_mcuenta_corriente_div_mcaja_ahorro := mcuenta_corriente / mcaja_ahorro ]
  dataset[ , AND_U_mcuenta_corriente_div_mprestamos_personales := mcuenta_corriente / mprestamos_personales ]
  dataset[ , AND_U_cpayroll_trx_div_mv_status01 := cpayroll_trx / mv_status01 ]
  dataset[ , AND_U_foto_mes_div_mpasivos_margen := foto_mes / mpasivos_margen ]
  dataset[ , AND_U_mcuentas_saldo_div_mdescubierto_preacordado := mcuentas_saldo / mdescubierto_preacordado ]
  ### Dos de yapa, Ulises ##
  dataset[, consumos_cred_diff_mpayroll := AND_monto_consumos_tarjetas_cred - mpayroll]  
  dataset[, consumos_cred_diff_mpayroll_div_mcuentas_saldo := (AND_monto_consumos_tarjetas_cred - mpayroll) / mcuentas_saldo ]
...
#Agrega todas las combinaciones de concientes entre vars
AgregarTasas <- function(campos_para_combinar){
  for( i in 1:length(campos_para_combinar) )
  {
    for(j  in 1:length(campos_para_combinar)){
      if(j!=i){
        dataset[, paste0(campos_para_combinar[i],'_div_',campos_para_combinar[j]) := get(campos_para_combinar[i]) / get(campos_para_combinar[j]) ]
      }
    }
  }
}
...
## funci�n que agrega rankings sin eliminar variables ori
AgregarRanking  <- function( cols_to_rank ) { 
...
AgregarRanking2  <- function( cols_to_rank ) { 
  ...
  for (col in cols_to_rank){
    dataset[ , paste0(col, '-rank') := rankear(get(col)), by= c("foto_mes")] 
  }
 #agrupo por mes
  ReportarCampos( dataset )
  #return(dataset_ranked)
...
CanaritosImportancia  <- function( canaritos_ratio=0.2, filter_450 = FALSE )
{
  gc()
  ReportarCampos( dataset )
  ...
  if ( ncol(dataset) > 450 & filter_450){
    print('borrar files de mas')
    importancias_to_filter = tb_importancia
    vars_imp = importancias_to_filter$Feature[!(importancias_to_filter$Feature %like% 'canarito')]
    vars_imp = vars_imp[1:450]
    vars_no_filtrar = c("foto_mes", "clase_ternaria", "numero_de_cliente",'mes')
...
dataset  <- fread( "./datasets/canaproxy_vidapropia_tasas_y_ranks.csv.gz")
gc()
...
cols_imp= c('ctrx_quarter','mcaja_ahorro','mtarjeta_visa_consumo','cpayroll_trx','mcuentas_saldo','ctarjeta_visa_transacciones','mpasivos_margen','mv_status01','mpayroll','mprestamos_personales','mcuenta_corriente','mdescubierto_preacordado','foto_mes','mactivos_margen')
if( palancas$agregartasas){ 
  AgregarTasas(cols_imp)
  cat('post_agregar_tasas')
  ReportarCampos(dataset)
}
...
if ( palancas$oriandrankings ){
  ...
  AgregarRanking2(cols_analiticas)
}
...
CanaritosImportancia( canaritos_ratio=0.1 , filter_450 = TRUE)
